...
def readxml(filename):
    ...
    VIAG_A = np.array([i.split(' ') for i in VIAG_A]).reshape(shape).astype(float)
    shape = (len([i.split(' ') for i in VIAG_Z])/23,23,23)
    VIAG_Z = np.array([i.split(' ') for i in VIAG_Z]).reshape(shape).astype(float)
